...
const HomeIcon = ({ active=false }: {active?: boolean}) => (
  <svg viewBox="0 0 24 24" className={`h-6 w-6 ${active ? "" : "opacity-70"}`} fill="currentColor">
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `group w-full h-full flex flex-col items-center justify-center gap-1 px-2 select-none
                     ${active || isActive ? "text-blue-600" : "text-gray-600 hover:text-gray-800"}`
                  }
                  aria-current={active ? "page" : undefined}
                >
                  <span aria-hidden className={`transition-transform ${active ? "scale-110" : "scale-100"}`}>
