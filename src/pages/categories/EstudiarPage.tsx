  ...
  return (
    ...
    >
        ...
        <div className="justify-center items-center text-center mb-5">
          <h1 className="text-[50px] font-black pb-1 leading-none tracking-[-0.03em]" style={{ color: 'var(--color-green)' }}>
            ESTUDIOS
          </h1>
          ...
          <div className="flex flex-wrap gap-2">
            {BADGE_CONFIG.map((badge) => (
              <BadgeWithIcon
                ...
                icon={
                  <img
                    src={`/icons/estudiar_icons/${badge.icon}`}
                    alt={badge.label}
                    className="w-5 h-5"
                  />
                }
                hoverIcon={
                  <img
                    src={`/icons/estudiar_icons/${badge.hoverIcon}`}
                    alt={badge.label}
                    className="w-5 h-5"
                  />
                }
        ...
        <FilterableContent isVisible={true}>
          ...
          <PlaceCards
            places={places}
            category="estudiar"
            title="Lugares para estudiar recomendados"
            loading={loading}
            error={error}
            onPlaceClick={(place) => {
              console.log("Lugar de estudio seleccionado:", place);
            }}
            itemsPerPage={6}
          />
          ...
          <div className="mt-10">
            <h2 className="text-2xl font-extrabold mb-2 text-[var(--color-text-primary)] text-center" style={{ color: 'var(--color-blue)' }}>
              Explora más lugares para estudiar
            </h2>
            <LocationMultiGrid
              places={places}
              loading={loading}
        ...
        {/* <FilterableContent isVisible={isFilterActive}>
          <FilteredResults
            ...
            onPlaceClick={(place) => {
              console.log("Lugar de estudio filtrado seleccionado:", place);
              if (activeFilter) {
