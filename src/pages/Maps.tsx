...
const Maps = () => {
  ...
  useEffect(() => {
    if (!navigationDestination || !userPosition) { setDirections(undefined); return; }
    const directionsService = new window.google.maps.DirectionsService();
    directionsService.route({ origin: userPosition, destination: navigationDestination.location, travelMode: window.google.maps.TravelMode.WALKING }, (result, status) => {
  ...
  const { displayTitle, displaySubtitle } = useMemo(() => {
    ...
    if (searchQuery) {
        if (loading) return { displayTitle: "Buscando...", displaySubtitle: searchQuery };
        if (placesToShow.length > 0) return { displayTitle: "Resultado", displaySubtitle: placesToShow[0].name };
        return { displayTitle: "Sin resultados", displaySubtitle: "Intenta otra búsqueda" };
    }
    if (selectedPlace) {
      return { displayTitle: `Seleccionado: ${selectedPlace.name}`, displaySubtitle: 'Presiona para navegar' };
    }
      ...
      <div className="relative h-[70vh]">
        ...
        <div className="absolute top-4 left-0 right-0 z-40 mx-auto w-11/12 md:w-3/4">
          <div className="flex items-center gap-2">
            <div className="flex-grow"><Search placeholder="Buscar lugares..." onSearch={setSearchQuery} onChange={setSearchQuery} value={searchQuery}/></div>
        ...
        <div className="w-full max-w-sm mx-auto px-4 flex flex-col gap-4">
          ...
          <div className="w-full flex items-center gap-2">
              <div className="flex-grow flex items-center justify-between rounded-full bg-[#073247] p-2 pl-5 text-white shadow-lg">
                  ...
                  <div className="pr-1">
                    <button onClick={handleResetLocation} className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-gray-800 shadow"><IoSync size={20} /></button>
                  </div>
              </div>
              {navigationDestination ? (
                ...
                <button onClick={handleStartNavigation} disabled={!selectedPlace || !userPosition} className="flex-shrink-0 flex h-12 w-12 items-center justify-center rounded-full bg-white text-[#073247] shadow-lg disabled:opacity-50"><IoNavigate size={24} /></button>
              )}
          </div>
            ...
            {navigationDestination ? (
              ...
              <div className="flex flex-col gap-6">
                ...
                <PlaceCards
                  ...
                  onPlaceClick={(place) => { console.log("Lugar seleccionado:", place); }}
                  itemsPerPage={6}
                />
              </div>
            )}
