...
export function useIntelligentFilters(config: Partial<FilterConfig> = {}) {
  ...
  const applyFilter = useCallback(
    (
      ...
      const filteredPlaces = places.filter((place) => {
        const analysis =
          place.ai_analysis?.categories[
            filter as keyof typeof place.ai_analysis.categories
          ];
        return (
          analysis?.detected &&
          analysis.confidence >= filterConfig.minConfidence
        );
      });
    ...
    (
      ...
      return [...places].sort((a, b) => {
        switch (sortBy) {
          ...
          case "relevance":
            const aRelevance =
              (a.ai_analysis?.overall_confidence || 0) * 0.7 +
              (a.rating || 0) * 0.3;
            const bRelevance =
              (b.ai_analysis?.overall_confidence || 0) * 0.7 +
              (b.rating || 0) * 0.3;
    ...
    (
      ...
      filters.forEach((filter) => {
        const filteredPlaces = places.filter((place) => {
          ...
          return (
            analysis?.detected &&
            analysis.confidence >= finalConfig.minConfidence
          );
        });
  ...
  const getHighConfidencePlaces = useCallback(
    (places: EnrichedPlace[], minConfidence: number = 0.8): EnrichedPlace[] => {
    ...
    (
      ...
      if (criteria.minRating !== undefined) {
        filteredPlaces = filteredPlaces.filter(
          (place) => place.rating && place.rating >= criteria.minRating!
        );
      }
      ...
      return sortedPlaces.slice(
        0,
        criteria.maxResults || finalConfig.maxResults
      );
    },
